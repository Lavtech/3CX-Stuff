        ...
        public override void Start()
        {
            Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        ext.CurrentProfile = ext.FwdProfiles.First(x=>x.Name.Equals("Out of office"));
                        ext.Save();
                        MyCall.Info($"Extension {ext.Number} is set to {ext.CurrentProfile.Name} status");
                        try
                        {
                            await MyCall.AssureMedia().ContinueWith(_ => MyCall.PlayPrompt(ext.GetPropertyValue("PROMPTSETID"), new[] { "ST_OUTOFFICE_SET" }, PlayPromptOptions.Blocked)).Unwrap();
                        }
                        catch(Exception ex)
                        {
