...
namespace OMSamples
{
    class PhoneSystemLog : PhoneSystem.ILog
    {
        public TextWriter Destination { get; init; } = Console.Out;
        public void Critical(string mess, params object[] varArgs)
        {
            Destination.WriteLine($"{DateTime.Now.TimeOfDay}|CRT:{mess}", varArgs);
            Destination.Flush();
        }
        ...
        {
            Destination.WriteLine($"{DateTime.Now.TimeOfDay}|ERR:{mess}", varArgs);
            Destination.Flush();
        }

        public void Exception(Exception ex)
        {
            Destination.WriteLine($"{DateTime.Now.TimeOfDay}|EXC:{ex}");
            Destination.Flush();
        }

        public void Info(string mess, params object[] varArgs)
        {
            Destination.WriteLine($"{DateTime.Now.TimeOfDay}|INF:{mess}", varArgs);
            Destination.Flush();
        }

        public void Trace(string mess, params object[] varArgs)
        {
            Destination.WriteLine($"{DateTime.Now.TimeOfDay}|TRC:{mess}", varArgs);
            Destination.Flush();
        }
    }
    ...
    class Program
    {
        ...
        {
            PhoneSystem.Logger = new PhoneSystemLog() { Destination = new StreamWriter($"PMSamples.log.txt", true) };
